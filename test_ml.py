...
def setup_data():
    ...
    X_train_processed, y_train_processed, encoder, lb = process_data(
        train, categorical_features=['workclass', 'education', 'marital-status', 'occupation', 'relationship', 'race', 'sex'], label='salary'
    )
...
def setup_test_data(setup_data):
    ...
    X_test_processed, y_test_processed, _, _ = process_data(
        test, categorical_features=['workclass', 'education', 'marital-status', 'occupation', 'relationship', 'race', 'sex'], label='salary', encoder=encoder, lb=lb, training=False
    )
